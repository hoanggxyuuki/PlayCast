...
export async function getSoundCloudRelatedTracks(trackId: string): Promise<SoundCloudResult[]> {
    try {
        ...
        // Use proxy server to avoid 401/403 errors from SoundCloud API
        const PROXY_SERVER = 'https://bidev.nhhoang.io.vn';
        const relatedUrl = `${PROXY_SERVER}/related?id=${trackId}&limit=10`;
        ...
        console.log(`[SoundCloud] Found ${results.length} related tracks via proxy`);
        return results;
    } catch (error: any) {
