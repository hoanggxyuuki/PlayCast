...
import { Channel, VideoPlayerProps } from '../../types';
...
export const AdvancedVideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  useEffect(() => {
    const fetchRelatedContent = async () => {
      // Use activeChannel (current playing) instead of channel (initial prop)
      ...
      try {
        if (isYouTube) {
          ...
          if (settings.autoPlayNext) {
            ...
            console.log('[YouTube] Found', related.length, 'related videos');
            if (related.length > 0) {
    ...
    fetchRelatedContent();
  }, [activeChannel?.id]); // Re-run when activeChannel changes
  ...
  const onVideoError = async (error: any) => {
    console.error('Video error:', error);

    // Check if it's a 403 error (expired stream URL)
    const errorCode = error?.error?.errorCode;
    const errorString = error?.error?.errorString || '';
    const is403Error = errorCode === '22004' || errorString.includes('403') || errorString.includes('BAD_HTTP_STATUS');

    // Try to refresh stream URL on 403 for online sources
    if (is403Error && activeChannel?.sourceType && activeChannel?.sourceUrl) {
      console.log('[Player] 403 error - attempting to refresh stream URL...');

      try {
        let newStreamUrl: string | null = null;

        if (activeChannel.sourceType === 'youtube') {
          const videoId = activeChannel.id.replace('youtube-', '');
          ...
          if (result && typeof result === 'object' && 'streamUrl' in result) {
            newStreamUrl = result.streamUrl;
          }
        }

        if (newStreamUrl && newStreamUrl !== activeChannel.url) {
          console.log('[Player] Got fresh stream URL, retrying...');
          setActiveChannel({
            ...activeChannel,
            url: newStreamUrl,
          });
        ...
        console.error('[Player] Failed to refresh stream:', refreshError);
      }
    }
  ...
  const playNextVideo = async () => {
    ...
    try {
      ...
      if (isYouTube) {
        ...
        title = ytVideo.title;
        sourceUrl = `https://www.youtube.com/watch?v=${ytVideo.videoId}`;
      } else {
      ...
      const newChannel: Channel = {
        ...
        sourceUrl: sourceUrl,
        sourceType: isYouTube ? 'youtube' : 'soundcloud',
      };

      console.log('[Player] Auto-playing next:', newChannel.name);
      setActiveChannel(newChannel);
      setCurrentTime(0);
      setIsPlaying(true);
      setNextVideo(null); // Clear to fetch new related content
    } catch (error) {
      console.error('[Player] Failed to play next video:', error);
      setIsLoading(false);
    }
  };
